...
local CreationGui = script.Parent.Parent.Main.EditRoom
...
local function OnCreateButtonClick()
	...
	if Errors then
		...
		AlertModule.Alert1("Error while editing room (Step "..StepNum..")", ErrorContent)
		LaunchStep(StepNum)
		return
	end
...
local RoomTypeModule = require(game.ReplicatedStorage.RoomType)
type RoomClass = RoomTypeModule.RoomClass

local function FillSteps(Info : RoomClass)
	print(Info)
	StepsModules[1].FillStep(Info)
	StepsModules[2].FillStep(Info)
	StepsModules[3].FillStep(Info)
	StepsModules[4].FillStep(Info)
end
...
function module.EnterRoomCreation(Info)
	...
	LaunchStep(1, true)
	FillSteps(Info)
	WindowsManagerModule.FetchWindow("EditRoom"):Open("Slide")
